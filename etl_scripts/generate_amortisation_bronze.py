        ...
        with open(csv_f, "r") as f:
            ...
            for i, line in enumerate(csv.reader(f)):
                if i == 0:
                    ...
                    col_names[
                        0
                    ] = "AS3"  # visible error from EDW where first columns has different names
                elif i == 1:
                    continue
                else:
                    if len(line) == 0:
            ...
            df = (
                ...
                .withColumn("checksum", F.md5(F.concat(F.col("ed_code"), F.col("AS3"))))
            )
            list_dfs.append(df)
...
def main():
    ...
    raw_amortisation_df = create_source_dataframe(
        run_props["SPARK"], all_amortisation_files
    )
