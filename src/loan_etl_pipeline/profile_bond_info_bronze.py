...
def profile_bond_info_bronze(spark, bucket_name, upload_prefix, file_key):
    """
    ...
    """
    logger.info("Start BOND INFO BRONZE PROFILING job.")
    ...
    if len(all_new_files) == 0:
        ...
        logger.info(f"Retrieved {len(all_new_files)} bond info data CSV files.")
        ...
        if dirty_files == []:
            ...
            bucket = storage_client.get_bucket(bucket_name)
            bucket.blob(
                f'dirty_dump/{datetime.date.today().strftime("%Y-%m-%d")}dirty_bond_info.csv'
            ).upload_from_string(dirty_df.to_csv(), "text/csv")
        if clean_files == []:
            logger.info("No passed CSV found. Workflow stopped!")
            sys.exit(1)
    logger.info("End BOND INFO BRONZE PROFILING job.")
    return clean_files
