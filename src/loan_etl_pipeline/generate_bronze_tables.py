...
def generate_bronze_tables(
    ...
    if len(all_files) == 0:
        ...
        file_sets = get_file_sets(all_files)
        for ed_code, ed_files in file_sets.items():
            logger.info(f"Create NEW {ed_code} dataframe")
            pcds, new_df = create_dataframe(spark, raw_bucketname, ed_files, data_type)
            if new_df is None:
                ...
                old_df = get_old_df(
                    spark, data_bucketname, bronze_prefix, pcds, data_type
                )
                if old_df is None:
