    ...
    for csv_f in all_files:
        ...
        with open(dest_csv_f, "r") as f:
            ...
            df = (
                ...
                .replace("", None)
                .withColumn("year", F.year(F.col("AS3")))
                .withColumn("month", F.month(F.col("AS3")))
                .withColumn(
...
def perform_scd2(spark, source_df, target_df):
    ...
    :param spark: SparkSession object.
    """
    source_df.createOrReplaceTempView("delta_table_amortisation")
    target_df.createOrReplaceTempView("staged_update")
    update_qry = """
        SELECT NULL AS mergeKey, source.*
        FROM delta_table_amortisation AS target
...
def generate_amortisation_bronze(spark, bucket_name, bronze_prefix, all_files):
    ...
    if len(all_files) == 0:
        ...
        if old_amortisation_df is None:
            logger.info("Initial load into AMORTISATION BRONZE")
            (
            ...
            logger.info("Upsert data into AMORTISATION BRONZE")
            perform_scd2(spark, old_amortisation_df, new_amortisation_df)

    logger.info("End AMORTISATION BRONZE job.")
    return 0
