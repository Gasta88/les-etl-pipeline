...
def get_old_df(spark, bucket_name, prefix, pcds):
    """
    Return BRONZE COLLATERAL table, but only the partitions from the specified pcds.
    ...
    if sum(check_list) > 0:
        ...
        df = (
            ...
            .load(f"gs://{bucket_name}/{prefix}/collaterals")
...
def perform_scd2(spark, source_df, target_df):
    ...
    :param spark: SparkSession object.
    """
    source_df.createOrReplaceTempView("delta_table_collateral")
    target_df.createOrReplaceTempView("staged_update")
    update_qry = """
        SELECT NULL AS mergeKey, source.*
        FROM delta_table_collateral AS target
    ...
    spark.sql(
        f"""
        MERGE INTO delta_table_collateral tgt
...
def generate_collateral_bronze(spark, bucket_name, bronze_prefix, all_files):
    """
    ...
    """
    logger.info("Start COLLATERALS BRONZE job.")
    ...
    if len(all_files) == 0:
        ...
        logger.info(f"Retrieved {len(all_files)} collateral data CSV files.")
        pcds, new_collateral_df = create_dataframe(spark, bucket_name, all_files)

        logger.info(f"Retrieve OLD dataframe. Use following PCDs: {pcds}")
        old_collateral_df = get_old_df(
            spark,
            bucket_name,
            bronze_prefix,
            pcds,
        )
        if old_collateral_df is None:
            logger.info("Initial load into COLLATERAL BRONZE")
            (
            ...
            logger.info("Upsert data into COLLATERAL BRONZE")
            perform_scd2(spark, old_collateral_df, new_collateral_df)

    logger.info("End COLLATERALS BRONZE job.")
    return 0
