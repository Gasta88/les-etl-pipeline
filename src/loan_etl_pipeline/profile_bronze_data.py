...
def profile_bronze_data(
    spark, raw_bucketname, data_bucketname, raw_prefix, file_key, data_type
):
    """
    ...
    :param raw_prefix: specific bucket prefix from where to collect source files.
    ...
    clean_dump_csv = bucket.blob(
        f'clean_dump/{datetime.date.today().strftime("%Y-%m-%d")}clean_{data_type}.csv'
    )
    if clean_dump_csv.exists():
        ...
        ed_codes = get_deal_codes(raw_bucketname, raw_prefix)
        if len(ed_codes) == 0:
            logger.warning("No deal found. Workflow stopped!")
            sys.exit(1)
        else:
            logger.info(f"Found {len(ed_codes)} deals.")
            for ed_code in get_deal_codes(raw_bucketname, raw_prefix):
                logger.info(f"Create NEW {ed_code} dataframe")
                all_new_files = get_csv_files(
                    raw_bucketname, f"{raw_prefix}/{ed_code}", file_key, data_type
                )
                if len(all_new_files) == 0:
                    ...
                    logger.info(
                        f"Retrieved {len(all_new_files)} {data_type} data CSV files."
                    )
                    dirty_files = []
                    clean_files = []
