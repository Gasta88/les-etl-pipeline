...
def profile_amortisation_bronze(spark, bucket_name, upload_prefix, file_key):
    """
    ...
    """
    logger.info("Start AMORTISATION BRONZE PROFILING job.")
    ...
    if len(all_new_files) == 0:
        ...
        if dirty_files == []:
            ...
            bucket = storage_client.get_bucket(bucket_name)
            bucket.blob(
                f'dirty_dump/{datetime.date.today().strftime("%Y-%m-%d")}dirty_amortisation.csv'
            ).upload_from_string(dirty_df.to_csv(), "text/csv")
        if clean_files == []:
            logger.info("No passed CSV found. Workflow stopped!")
            sys.exit(1)
    logger.info("End AMORTISATION BRONZE PROFILING job.")
    return clean_files
