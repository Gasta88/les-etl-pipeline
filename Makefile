...
run_asset_bronze: ## Run the dataproc serverless job
	...
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=mini_source --target-prefix=SME/bronze/assets --file-key=Loan_Data --stage-name=bronze_asset
...
run_collateral_bronze: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=mini_source --target-prefix=SME/bronze/collaterals --file-key=Collateral --stage-name=bronze_collateral
...
run_bond_info_bronze: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=mini_source --target-prefix=SME/bronze/bond_info --file-key=Bond_Info --stage-name=bronze_bond_info
...
run_amortisation_bronze: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=mini_source --target-prefix=SME/bronze/amortisation --file-key=Amortization --stage-name=bronze_amortisation
...
run_deal_details_bronze: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=mini_source --target-prefix=SME/bronze/deal_details --file-key=Deal_Details --stage-name=bronze_deal_details
...
run_asset_silver: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=SME/bronze/assets --target-prefix=SME/silver/assets --pcds="" --stage-name=silver_asset
...
run_collateral_silver: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=SME/bronze/collaterals --target-prefix=SME/silver/collaterals --pcds="" --stage-name=silver_collateral
...
run_bond_info_silver: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=SME/bronze/bond_info --target-prefix=SME/silver/bond_info --pcds="" --stage-name=silver_bond_info
...
run_amortisation_silver: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=SME/bronze/amortisation --target-prefix=SME/silver/amortisation --pcds="" --stage-name=silver_amortisation
...
run_deal_details_silver: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=SME/bronze/deal_details --target-prefix=SME/silver/deal_details --pcds="" --stage-name=silver_deal_details
...
run_quandl_silver: ## Run the dataproc serverless job
	...
	--metastore-service=projects/${PROJECT_ID}/locations/${REGION}/services/data-catalog-${PROJECT_ID} \
	--version=2.0 \
	-- --project=${PROJECT_ID} --raw-bucketname=${RAW_BUCKET} --data-bucketname=${DATA_BUCKET} --source-prefix=mini_source --target-prefix=externals/silver/quandl --pcds="" --stage-name=silver_quandl